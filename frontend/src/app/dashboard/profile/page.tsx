...
function generateInitials(username: string | undefined, address: string): string {
  if (username && username.length > 0) {
    ...
    if (parts.length >= 2) {
      ...
      return parts[0].substring(0, 2).toUpperCase().replace(/[^A-Z]/g, "");
    }
  }
...
function formatDisplayName(username: string | undefined, address: string): string {
  if (username && username.length > 0) {
...
export default function ProfilePage() {
  ...
  const { data: userPredictions, isLoading: isLoadingPredictions } = useUserPredictions();
  ...
  }, [userStats, userPredictions, userRank]);
  ...
  const userData = useMemo(() => {
    ...
    const formattedUsername = username && username.length > 0
  ...
  const isLoading = isLoadingStats || isLoadingUsername || isLoadingPredictions;
          ...
          <main className="flex-1 lg:ml-64 p-4 sm:p-6 lg:p-8">
            <Card className="bg-[#1E293B] border-dark-700">
              <CardContent className="p-12 text-center">
                <p className="text-gray-400 mb-2">Please connect your wallet to view your profile</p>
              </CardContent>
            </Card>
          </main>
        </div>
